...
class BLEScanner {
    ...
    /**
     * Inizializza lo scanner BLE
     * @returns {Promise<boolean>} true se l'inizializzazione ha successo
     */
    async initialize() {
        try {
            // Utilizziamo la classe BLEConnection per l'inizializzazione
     ...
     * @returns {Promise<Object|null>} Il dispositivo trovato o null
     */
    async findTargetDevice() {
        try {
            ...
            this.eventManager.emit('scanner:search_started', {
                targetName: settings.TARGET_DEVICE.NAME,
                timestamp: getCurrentTimestamp()
            });
                    ...
                    try {
                        ...
                        if (name === settings.TARGET_DEVICE.NAME) {
                            if (rssi && rssi >= settings.TARGET_DEVICE.SIGNAL_STRENGTH_THRESHOLD) {
                                ...
                                this.eventManager.emit('scanner:device_found', {
                                    name,
                                    address,
                                    rssi,
                                    timestamp: getCurrentTimestamp()
                                ...
                                this.eventManager.emit('scanner:weak_signal', {
                                    name,
                                    address,
                                    rssi,
                                    threshold: settings.TARGET_DEVICE.SIGNAL_STRENGTH_THRESHOLD,
                                    timestamp: getCurrentTimestamp()
                                });
                            }
                        }
            ...
            this.eventManager.emit('scanner:search_timeout', {
                targetName: settings.TARGET_DEVICE.NAME,
                duration: settings.TARGET_DEVICE.SCAN_TIMEOUT,
                timestamp: getCurrentTimestamp()
            });
            ...
            this.eventManager.emit('scanner:search_error', {
                error: error.message,
                timestamp: getCurrentTimestamp()
            });
    ...
    /**
     * Tenta di connettersi automaticamente al dispositivo target
     * @returns {Promise<boolean>} true se la connessione ha successo
     */
    async autoConnectToTarget() {
        try {
            ...
            this.eventManager.emit('scanner:connect_failed', {
                error: error.message,
                attempts: settings.TARGET_DEVICE.RETRY_ATTEMPTS,
                timestamp: getCurrentTimestamp()
            });
     ...
     * @param {number} scanDuration - Durata della scansione in millisecondi
     * @returns {Promise<Array>} Lista dei dispositivi trovati
     */
    async startScan(scanDuration = 10000) {
        ...
        try {
                    ...
                    for (const address of devices) {
                        if (!this.discoveredDevices.has(address)) {
                            try {
                                ...
                                this.eventManager.emit('scanner:device_discovered', {
                                    ...deviceInfo,
                                    timestamp: getCurrentTimestamp()
                                });
                            } catch (error) {
            ...
            this.eventManager.emit('scanner:scan_error', {
                error: error.message,
                timestamp: getCurrentTimestamp()
            });
    ...
    async stopScan() {
        ...
        try {
            ...
            this.eventManager.emit('scanner:stop_error', {
                error: error.message,
                timestamp: getCurrentTimestamp()
            });
        }
    ...
    /**
     * Avvia la discovery in modo sicuro
        ...
        try {
            ...
            for (const charUUID of characteristics) {
                try {
                    ...
                    this.eventManager.emit('scanner:notifications_enabled', {
                        characteristicUUID: charUUID,
                        timestamp: getCurrentTimestamp()
                    });
                    ...
                    this.eventManager.emit('scanner:characteristic_error', {
                        characteristicUUID: charUUID,
                        error: error.message,
                        timestamp: getCurrentTimestamp()
                    });
            ...
            this.eventManager.emit('scanner:setup_error', {
                deviceAddress: device.address,
                error: error.message,
                timestamp: getCurrentTimestamp()
            });
    ...
    async disconnect() {
        if (this.targetDevice && this.isConnected) {
            try {
                ...
                this.eventManager.emit('scanner:disconnect_error', {
                    error: error.message,
                    timestamp: getCurrentTimestamp()
                });
     ...
     * @param {Buffer} buffer - Buffer dei dati
    ...
    /**
     * Restituisce l'elenco dei dispositivi scoperti
     * @returns {Array} Lista dei dispositivi
     */
    ...
    /**
     * Assicura che la discovery sia fermata
     * @returns {Promise<void>}
     */
